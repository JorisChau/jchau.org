...
author: Joris Chau
date: '2021-10-11T19:20:00+02:00'
...
summary: ''
authors: []
lastmod: '2021-10-11T19:20:00+02:00'
featured: no
image:
  caption: ''
  focal_point: ''
  ...
  tryfit <- tryCatch({
    suppressWarnings(
      gsl_nls(
        ...
        control = list(maxiter = 1e4, scale = scale, solver = solver),
...
plot_heatmap <- function(gsl_fits, nls_fits, title, legend) {
  ...
  gsl_iter <- ggplot(gsl_data, aes(x = control, y = algorithm, fill = niter)) + 
    ...
    guides(fill = if(legend) guide_colorbar(title = "# iterations") else FALSE) 
  ...
  gsl_status <- ggplot(gsl_data, aes(x = control, y = algorithm, fill = status)) +
    ...
    scale_y_discrete(limits = rev(levels(gsl_data$algorithm))) +
    scale_x_discrete(guide = guide_axis(angle = 45)) +
    guides(fill = if(legend) guide_legend(title = "") else FALSE)
...
The recent [`gslnls`](https://CRAN.R-project.org/package=gslnls)-package provides R bindings to nonlinear least-squares optimization with 
the [GNU Scientific Library (GSL)](https://www.gnu.org/software/gsl/) using the trust region methods implemented by the `gsl_multifit_nlinear` module. This module was added in GSL version 2.2 (released in August 2016) and the available nonlinear-least squares routines have been thoroughly tested and are well documented, see [@gsl_manual].
...
Below, the individual observed datasets are displayed, with the (unique) predictor variable on the x-axis and the response variable on the y-axis. The overlayed continuous line corresponds to the regression model evaluated at the certified parameter values.
...
In order to benchmark the performance of the GSL nonlinear least-squares routines against several standard alternatives in R, each nonlinear regression model is also fitted using the standard `nls()` function, as well as the `nlsLM()` function from the `minpack.lm`-package. 
...
The GSL model fits are produced with `gsl_nls()` in the `gslnls`-package. A minimal function call consists of the model `formula`, the data and a set of starting values. By default, `gsl_nls()` uses the Levenberg-Marquadt algorithm with control parameters `scale = "more"` and `solver = "qr"`. The starting values $(b_1 = 100, b_2 = 1, b_3 = 0.1)$ are taken from Table 1.

```{r}
library(NISTnls)    ## NIST nls datasets
library(gslnls)     ## GSL bindings
...
The object returned by `gsl_nls()` inherits from class `"nls"`. For this reason, all generic functions available for objects of class `"nls"` are also applicable to objects fitted with `gsl_nls()`. For instance,
...
The `predict` method also allows for the calculation of asymptotic confidence and prediction (tolerance) intervals in addition to prediction of the expected response as returned by the standard `predict.nls` method:
...
Based on the displayed results, an initial observation is that the default Gauss-Newton algorithm in `nls()` fails to produce *any* successful model fit and returns an error for each selected test problem, which is similar to the worked out `Rat42` example in the previous section. The Port and (`minpack.lm`) Levenberg-Marquadt algorithms show roughly similar performance, but only converge successfully for half of the test problems. The p-linear algorithm is somewhat special as it is only applicable for regression models that can be factored as a partially linear model. However, if applicable, the p-linear algorithm can be a powerful alternative as demonstrated by the `BoxBOD` problem, where most other (general) NLS routines fail to converge. More precisely, the `BoxBOD` regression model contains only two parameters, and by factoring out the linear parameter, the nonlinear model fit that needs to be optimized by the p-linear algorithm depends only on a single unknown parameter.

Regarding the GSL routines, for each test problem there exists *at least* one algorithm configuration that is able to produce a successful model fit. Across test problems and control parameter configurations, the GSL Levenberg-Marquadt algorithms with and without geodesic acceleration (`lm`, `lmaccel`, `lmaccel+fvv`) appear to be the most stable, as also seen in the figure below, which displays the total number of successful model fits across test problems. In comparison to the LM algorithm without geodesic acceleration (`lm`), the LM algorithm with geodesic acceleration (`lmaccel`, `lmaccel+fvv`) fails to converge in the `Rat43` problem, but on the other hand the `lmaccel+fvv` algorithm is the *only* GSL routine that is able to produce a successful fit in the `BoxBOD` problem.

Across control parameter configurations, in terms of the scaling method, `more` rescaling (the default) appears to be the most stable, followed by `marqaudt` rescaling and `levenberg` rescaling. In the figure below, this is seen most prominently for the different variations of the Dogleg algorithm (`dogleg`, `ddogleg`, `subspace2D`) and somewhat less for the Levenberg-Marquadt algorithms. In terms of the chosen solver method, the `svd` solver produces slightly more robust results than the `qr` and `cholesky` solver methods, which seems to be in line with the solver method section in the GSL documentation.
...
gsl_success <- ggplot(gsl_data, aes(x = control, y = algorithm, fill = count)) + 
  scale_fill_fermenter(direction = 1, palette = "Spectral", breaks = 0:7, limits = c(0, 7), na.value = "grey70") +
  scale_y_discrete(limits = rev(levels(gsl_data$algorithm))) +
  scale_x_discrete(guide = guide_axis(angle = 45)) +
  guides(fill = guide_colorbar(title = "# Successful fits")) + 
  geom_tile(colour = "white") + 
  coord_equal() + 
...
As supplementary information, below we display the required number of iterations to reach convergence for each successfully converged NLS routine. In case of a successful model fit, the Port algorithm requires only a small number of iterations to reach convergence. The number of iterations required by the `minpack.lm` Levenberg-Marquadt algorithm and GSL Levenberg-Marquadt algorithm(s) is of the same order of magnitude as one would expect. Among the GSL routines, except for the `MGH09` problem, the general tendency appears to be that the Dogleg-based algorithms (`dogleg`, `ddogleg`, `subspace2D`) require less iterations than the LM-based algorithms. This is illustrated most clearly by the `Rat42` and `Bennet5` plots.
